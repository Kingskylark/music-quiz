 ...
 */
function verify_account_paystack($account_number, $bank_code) {
...
/**
 * Send payment via Paystack Transfer
 */
function process_payment_paystack($payment_id) {
    global $conn;
    
    // Get payment details
    $query = "SELECT p.*, u.name FROM payments p 
    ...
    $recipient_data = [
        ...
        'bank_code' => $payment['bank_name'], // This should be bank code
        'currency' => 'NGN'
    ];
    ...
    $response = curl_exec($ch);
    $recipient_result = json_decode($response, true);
    
    if (!$recipient_result || !$recipient_result['status']) {
        return ['success' => false, 'message' => 'Could not create recipient'];
    }
    ...
    $transfer_data = [
        'source' => 'balance',
        'amount' => $payment['prize_amount'] * 100, // Convert to kobo
        'recipient' => $recipient_code,
        'reason' => "Music Quiz Prize - Rank {$payment['rank']}"
    ];
    ...
    if ($transfer_result && $transfer_result['status'] === true) {
